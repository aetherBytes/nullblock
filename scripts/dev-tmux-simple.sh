...
tmux send-keys -t $SESSION_NAME:infrastructure.0 "brew services start postgresql@15" Enter
...
tmux send-keys -t $SESSION_NAME:backend.0 "cd svc/nullblock-mcp && python3 -m mcp.server" Enter
tmux send-keys -t $SESSION_NAME:backend.1 "cd svc/nullblock-orchestration && python3 -m orchestration.server" Enter
tmux send-keys -t $SESSION_NAME:backend.2 "cd svc/nullblock-agents && python3 -m agents.server" Enter
...
tmux send-keys -t $SESSION_NAME:rust.0 "cd svc/erebus && cargo run" Enter
...
tmux send-keys -t $SESSION_NAME:frontend.0 "cd svc/hecate && npm run develop" Enter
tmux send-keys -t $SESSION_NAME:frontend.1 "echo 'Frontend logs will appear here...'" Enter
...
tmux send-keys -t $SESSION_NAME:monitoring.0 "echo 'MCP: http://localhost:8000/health'" Enter
tmux send-keys -t $SESSION_NAME:monitoring.0 "echo 'Orchestration: http://localhost:8001/health'" Enter
tmux send-keys -t $SESSION_NAME:monitoring.0 "echo 'Agents: http://localhost:8002/health'" Enter
tmux send-keys -t $SESSION_NAME:monitoring.0 "echo 'Erebus: http://localhost:8003/health'" Enter
tmux send-keys -t $SESSION_NAME:monitoring.0 "echo 'Frontend: http://localhost:5173'" Enter
tmux send-keys -t $SESSION_NAME:monitoring.1 "echo 'Service logs will appear here...'" Enter
