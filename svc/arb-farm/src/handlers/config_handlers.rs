...
pub async fn set_risk_level(
    ...
    tracing::info!(
        "✅ Synced {} strategies with new risk config (SL={}%, TP={}%, Trail={}%, Time={}min)",
        synced_count,
        params.stop_loss_pct,
...
pub async fn get_risk_level(State(state): State<AppState>) -> impl IntoResponse {
...
pub async fn set_custom_risk(
    ...
    tracing::info!(
        "✅ Synced {} strategies with custom risk config",
        synced_count
    );
