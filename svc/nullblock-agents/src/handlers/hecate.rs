...
pub async fn chat(
    ...
    let user_context = if is_dev {
        ...
        ctx.insert(
            ...
            json!(FREE_TIER_MAX_OUTPUT_TOKENS),
        );
        info!(
            "🆓 Free tier user: limiting output to {} tokens",
    ...
    if let (Some(ref uid), Some(_)) = (&user_id, &rate_limit_info) {
        if let Err(e) = state
            .erebus_client
            .increment_rate_limit(uid, "hecate")
            .await
        {
        ...
        match factory.health_check().await {
            ...
            Err(e) => {
                return Err(AppError::LLMRequestFailed(format!(
                    "LLM service health check failed: {}. Please check your API key configuration.",
                    e
                )));
            }
        }
...
pub async fn available_models(
    ...
    let available_models = if let Some(openrouter_key) = &api_keys.openrouter {
        ...
        match fetch_openrouter_models(openrouter_key).await {
            Ok(models) => {
                info!(
                    "✅ OpenRouter API call successful, got {} models",
                    models.len()
                );
                models
            }
            Err(e) => {
                error!(
                    "❌ OpenRouter API call failed: {}, falling back to mock models",
                    e
                );
                get_fallback_models()
            }
...
async fn fetch_openrouter_models(api_key: &str) -> Result<Vec<serde_json::Value>, AppError> {
    ...
    for model in models_array {
        if let Some(model_obj) = model.as_object() {
            ...
            let description = model_obj
                ...
                .and_then(|v| v.as_str())
                .unwrap_or("");
            let context_length = model_obj
                .get("context_length")
                .and_then(|v| v.as_u64())
                .unwrap_or(8192);
            let canonical_slug = model_obj
                .get("canonical_slug")
                .and_then(|v| v.as_str())
                .unwrap_or("");
            let hugging_face_id = model_obj
                .get("hugging_face_id")
                .and_then(|v| v.as_str())
                .unwrap_or("");

            // Extract created timestamp (Unix timestamp) - ensure it's recent for testing
            let created = model_obj
                .get("created")
                .and_then(|v| v.as_i64())
                .unwrap_or_else(|| {
            ...
            let output_modalities = architecture
                ...
                .map(|arr| arr.iter().filter_map(|v| v.as_str()).collect::<Vec<_>>())
                .unwrap_or_else(|| vec!["text"]);
            let tokenizer = architecture
                .get("tokenizer")
                .and_then(|v| v.as_str())
                .unwrap_or("");
            let instruct_type = architecture
                .get("instruct_type")
                .and_then(|v| v.as_str())
                .unwrap_or("");

            // Extract top_provider information
            let top_provider = model_obj
                .get("top_provider")
                .unwrap_or(&serde_json::Value::Null);
            let is_moderated = top_provider
                .get("is_moderated")
            ...
            let (
                ...
                let web_search = pricing_obj
                    ...
                    .and_then(|s| s.parse::<f64>().ok())
                    .unwrap_or(0.0);
                let internal_reasoning = pricing_obj
                    .get("internal_reasoning")
                    .and_then(|v| v.as_str())
                    .and_then(|s| s.parse::<f64>().ok())
                    .unwrap_or(0.0);
                let input_cache_read = pricing_obj
                    .get("input_cache_read")
                    .and_then(|v| v.as_str())
                    .and_then(|s| s.parse::<f64>().ok())
                    .unwrap_or(0.0);
                let input_cache_write = pricing_obj
            ...
            let supported_parameters = model_obj
                .get("supported_parameters")
                .and_then(|v| v.as_array())
                .map(|arr| arr.iter().filter_map(|v| v.as_str()).collect::<Vec<_>>())
                .unwrap_or_default();
            let per_request_limits = model_obj
    ...
    let recent_models = processed_models
        .iter()
        .filter(|model| {
    ...
    info!(
        "🔍 Found {} models matching newer model keywords (sonoma, qwen, kimi, dusk, sky)",
        found_newer_models.len()
    );
...
pub async fn search_models(
    ...
    let available_models = if let Some(openrouter_key) = &api_keys.openrouter {
        fetch_openrouter_models(openrouter_key)
            .await
...
pub async fn set_model(
    ...
    if is_free_tier && !is_dev {
        ...
        if let Some(ref llm_factory) = agent.llm_factory {
            let factory = llm_factory.read().await;
            if let Err(msg) = factory
                .validate_model_for_free_tier(&request.model_name)
                .await
            {
                drop(factory);
                drop(agent);
            ...
            info!(
                "✅ Model '{}' validated for free-tier user",
                request.model_name
            );
        }
        drop(agent);
    ...
    if success {
        ...
        let error_detail = agent
            .get_model_availability_reason(&request.model_name, &api_keys)
            .await;
...
pub async fn get_tools(State(state): State<AppState>) -> Result<Json<serde_json::Value>, AppError> {
    ...
    Ok(Json(json!({
        ...
        "timestamp": chrono::Utc::now().to_rfc3339(),
        "data": {
            "external_mcp": external_tools,
            "hecate_tools": local_tools_json,
            "hecate_tool_names": local_tools_json.iter()
