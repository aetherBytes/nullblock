...
SELECT 
    id, source_identifier, chain, user_type, source_type, wallet_type,
    email, metadata, preferences, additional_metadata, is_active
FROM dblink('host=localhost port=5440 dbname=erebus user=postgres password=postgres_secure_pass',
...
SELECT 'Erebus users:' as database, COUNT(*) as count FROM dblink('host=localhost port=5440 dbname=erebus user=postgres password=postgres_secure_pass', 'SELECT COUNT(*) FROM user_references WHERE is_active = true') AS erebus_count(count bigint)
UNION ALL
