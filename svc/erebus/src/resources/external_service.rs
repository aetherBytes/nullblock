...
impl ExternalService {
    ...
    pub async fn call_hecate_with_cache(
        &self,
        endpoint: &str,
        use_cache: bool,
    ) -> Result<Value, Box<dyn std::error::Error + Send + Sync>> {
    ...
    pub async fn call_mcp_with_cache(
        &self,
        endpoint: &str,
        use_cache: bool,
    ) -> Result<Value, Box<dyn std::error::Error + Send + Sync>> {
    ...
    pub async fn hecate_set_model(
        ...
        if response.status().is_success() {
            ...
            Err(format!(
                "Failed to set Hecate model with status: {}",
                response.status()
            )
            .into())
        }
    ...
    pub async fn hecate_update_personality(
        &self,
        personality: Value,
    ) -> Result<Value, Box<dyn std::error::Error + Send + Sync>> {
        ...
        let response = self.client.post(&url).json(&personality).send().await?;
        ...
        if response.status().is_success() {
            ...
            Err(format!(
                "Failed to update Hecate personality with status: {}",
                response.status()
            )
            .into())
        }
    ...
    pub async fn hecate_clear_history(
        &self,
    ) -> Result<Value, Box<dyn std::error::Error + Send + Sync>> {
        ...
        if response.status().is_success() {
            ...
            Err(format!(
                "Failed to clear Hecate history with status: {}",
                response.status()
            )
            .into())
        }
    ...
    pub async fn hecate_get_history(
        &self,
        limit: Option<usize>,
    ) -> Result<Value, Box<dyn std::error::Error + Send + Sync>> {
        ...
        if response.status().is_success() {
            ...
            Err(format!(
                "Failed to get Hecate history with status: {}",
                response.status()
            )
            .into())
        }
    ...
    pub async fn hecate_search_models(
        ...
        if response.status().is_success() {
            ...
            Err(format!(
                "Failed to search Hecate models with status: {}",
                response.status()
            )
            .into())
        }
    ...
    pub async fn hecate_model_info(
        ...
        if response.status().is_success() {
            ...
            Err(format!(
                "Failed to get Hecate model info with status: {}",
                response.status()
            )
            .into())
        }
    ...
    pub async fn hecate_reasoning(
        ...
        if response.status().is_success() {
            ...
            Err(format!(
                "Failed to execute reasoning task with status: {}",
                response.status()
            )
            .into())
        }
    ...
    pub async fn hecate_restart(&self) -> Result<Value, Box<dyn std::error::Error + Send + Sync>> {
        ...
        if response.status().is_success() {
            ...
            Err(format!(
                "Failed to restart Hecate agent with status: {}",
                response.status()
            )
            .into())
        }
    ...
    pub async fn get_all_agents_info(
        &self,
    ) -> Result<Value, Box<dyn std::error::Error + Send + Sync>> {
        ...
        agents_info.insert(
            ...
            serde_json::json!(agents_info.len() - 2),
        ); // Exclude timestamp and total_services
    ...
    pub async fn coordinate_agents(
        ...
        info!(
            "🤝 Coordinating agents for task: {} with required agents: {:?}",
        ...
        for agent in required_agents {
            match *agent {
                "hecate" => match self.hecate_reasoning(task, None).await {
                    Ok(result) => {
                    ...
                    Err(e) => {
                        ...
                        coordination_results
                            .insert("hecate_error".to_string(), serde_json::json!(e.to_string()));
                    }
                },
                "mcp" => match self.call_mcp("execute").await {
                    Ok(result) => {
                    ...
                    Err(e) => {
                        ...
                        coordination_results
                            .insert("mcp_error".to_string(), serde_json::json!(e.to_string()));
                    }
                },
                _ => {
        ...
        coordination_results.insert(
            ...
            serde_json::json!(successful_agents),
        );
        coordination_results.insert(
            "failed_agents".to_string(),
            serde_json::json!(failed_agents),
        );
        coordination_results.insert(
            "timestamp".to_string(),
            serde_json::json!(chrono::Utc::now().to_rfc3339()),
        ...
        if failed_agents.is_empty() {
            ...
            warn!(
                "⚠️ Some agents failed during coordination for task: {}",
                task
            );
        }
    ...
    pub async fn monitor_agents_health(
        &self,
    ) -> Result<Value, Box<dyn std::error::Error + Send + Sync>> {
        ...
        health_data.insert(
            "monitoring_duration_ms".to_string(),
            serde_json::json!(start_time.elapsed().as_millis()),
        );
        health_data.insert(
            "monitoring_timestamp".to_string(),
            serde_json::json!(chrono::Utc::now().to_rfc3339()),
        );
    ...
    pub async fn batch_call_hecate(
        &self,
        endpoints: &[&str],
        ...
        for endpoint in endpoints {
            ...
            let task = tokio::spawn(async move { service.call_hecate(&endpoint_str).await });
    ...
    pub async fn detailed_health_check(&self) -> Value {
        ...
        services_health.insert(
            "total_check_time_ms".to_string(),
            serde_json::json!(start_time.elapsed().as_millis()),
        );
        services_health.insert(
            "note".to_string(),
            serde_json::json!("Direct service calls - no HTTP overhead"),
        );
        services_health.insert(
            "timestamp".to_string(),
            serde_json::json!(chrono::Utc::now().to_rfc3339()),
    ...
    pub async fn clear_expired_cache(&self) -> usize {
        ...
        cache.retain(|_, (_, timestamp)| timestamp.elapsed().as_secs() < self.cache_ttl_seconds);
            ...
            match self.client.get(url).send().await {
                Ok(response) => {
                    if response.status().is_success() {
                        match response.json::<Value>().await {
                            ...
                            Err(e) => {
                                ...
                                if attempt < self.max_retries {
                                    warn!(
                                        "⚠️ Failed to parse response on attempt {}, retrying...",
                                        attempt + 1
                                    );
                        ...
                        if attempt < self.max_retries {
                            warn!(
                                "⚠️ HTTP error {} on attempt {}, retrying...",
                                response.status(),
                                attempt + 1
                            );
                            sleep(Duration::from_millis(
