...
pub async fn handle_jsonrpc(
    ...
    let result = match request.method.as_str() {
            ...
            match serde_json::from_value::<InitializeRequest>(request.params.unwrap_or(json!({}))) {
                Ok(init_request) => {
                    match handlers::initialize(State(state.clone()), init_request).await {
                        Ok(Json(result)) => match serde_json::to_value(result) {
                            Ok(value) => Ok(value),
                            Err(e) => {
        ...
        "resources/list" => match handlers::list_resources(State(state.clone())).await {
            Ok(Json(result)) => match serde_json::to_value(result) {
                Ok(value) => Ok(value),
                Err(e) => {
        ...
        "tools/list" => match handlers::list_tools(State(state.clone())).await {
            Ok(Json(result)) => match serde_json::to_value(result) {
                Ok(value) => Ok(value),
                Err(e) => {
        ...
        "prompts/list" => match handlers::list_prompts(State(state.clone())).await {
            Ok(Json(result)) => match serde_json::to_value(result) {
                Ok(value) => Ok(value),
                Err(e) => {
