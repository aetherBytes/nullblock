...
use crate::protocols::a2a::handlers::{
    cancel_task, delete_push_notification_config, get_agent_card, get_push_notification_config,
    get_task, list_push_notification_configs, list_tasks, resubscribe_task, send_message,
    send_streaming_message, set_push_notification_config,
};
use crate::protocols::a2a::types::{
    JsonRpcError, JsonRpcRequest, JsonRpcResponse, MessageSendRequest, MessageStreamRequest,
    PushNotificationConfigRequest, TaskCancelRequest, TaskListRequest,
};
...
async fn handle_message_send(params: Option<Value>) -> Result<Value, JsonRpcError> {
    ...
    let request: MessageSendRequest = serde_json::from_value(params).map_err(|e| JsonRpcError {
        code: -32602,
    ...
    match send_message(Json(request)).await {
        Ok(Json(response)) => serde_json::to_value(response).map_err(|e| JsonRpcError {
            code: -32603,
...
async fn handle_tasks_get(state: AppState, params: Option<Value>) -> Result<Value, JsonRpcError> {
    ...
    match get_task(State(state), axum::extract::Path(task_id.to_string())).await {
        Ok(Json(task)) => serde_json::to_value(task).map_err(|e| JsonRpcError {
            code: -32603,
            message: format!("Internal error: {}", e),
            data: None,
        }),
...
async fn handle_tasks_cancel(
    ...
    let request: TaskCancelRequest = serde_json::from_value(params).map_err(|e| JsonRpcError {
        code: -32602,
        message: format!("Invalid parameters: {}", e),
        data: None,
    })?;
...
async fn handle_push_config_set(params: Option<Value>) -> Result<Value, JsonRpcError> {
    ...
    let request: PushNotificationConfigRequest =
        serde_json::from_value(params).map_err(|e| JsonRpcError {
            code: -32602,
            message: format!("Invalid parameters: {}", e),
            data: None,
    ...
    match set_push_notification_config(axum::extract::Path(task_id), Json(request)).await {
        Ok(Json(config)) => serde_json::to_value(config).map_err(|e| JsonRpcError {
            code: -32603,
            message: format!("Internal error: {}", e),
            data: None,
        }),
...
async fn handle_push_config_get(params: Option<Value>) -> Result<Value, JsonRpcError> {
    ...
    match get_push_notification_config(axum::extract::Path((
        task_id.to_string(),
        config_id.to_string(),
    )))
    .await
...
async fn handle_push_config_list(params: Option<Value>) -> Result<Value, JsonRpcError> {
    ...
    match list_push_notification_configs(axum::extract::Path(task_id.to_string())).await {
        Ok(Json(configs)) => serde_json::to_value(configs).map_err(|e| JsonRpcError {
            code: -32603,
            message: format!("Internal error: {}", e),
            data: None,
        }),
...
async fn handle_push_config_delete(params: Option<Value>) -> Result<Value, JsonRpcError> {
    ...
    match delete_push_notification_config(axum::extract::Path((
        task_id.to_string(),
        config_id.to_string(),
    )))
    .await
    {
        Ok(Json(result)) => serde_json::to_value(result).map_err(|e| JsonRpcError {
            code: -32603,
            message: format!("Internal error: {}", e),
            data: None,
        }),
...
async fn handle_get_agent_card(_params: Option<Value>) -> Result<Value, JsonRpcError> {
    match get_agent_card().await {
        Json(card) => serde_json::to_value(card).map_err(|e| JsonRpcError {
            code: -32603,
            message: format!("Internal error: {}", e),
            data: None,
        }),
