...
const ArbitrageDashboard: React.FC<ArbitrageDashboardProps> = ({ onClose }) => {
  ...
  const [activeTab, setActiveTab] = useState<'opportunities' | 'history' | 'metrics' | 'settings'>(
    'opportunities',
  );
  ...
  const handleExecuteTrade = async (opportunity: ArbitrageOpportunity) => {
    try {
      ...
      if (response.success) {
        ...
        alert(
          `Trade executed successfully! ${response.protected ? 'MEV protection was used.' : ''}`,
        );
      } else {
  ...
  const renderOpportunities = () => (
    <div className={styles.opportunitiesTab}>
      <div className={styles.tabHeader}>
        ...
        <button className={styles.refreshButton} onClick={loadOpportunities} disabled={loading}>
          ...
          opportunities.map((opp) => (
            <div key={opp.id} className={styles.opportunityCard}>
              <div className={styles.opportunityHeader}>
                ...
                <div
                  className={`${styles.confidence} ${opp.confidence > 0.7 ? styles.high : opp.confidence > 0.5 ? styles.medium : styles.low}`}
                >
              ...
              <div className={styles.opportunityDetails}>
                ...
                <div className={styles.metrics}>
                  ...
                  <div className={styles.metric}>
                    ...
                    <span
                      className={`${styles.value} ${opp.net_profit > 0 ? styles.profit : styles.loss}`}
                    >
      ...
      {metrics ? (
        <div className={styles.metricsGrid}>
          ...
          <div className={styles.metricCard}>
            <h4>Net Profit</h4>
            <div
              className={`${styles.metricValue} ${metrics.net_profit >= 0 ? styles.profit : styles.loss}`}
            >
          ...
          {history.map((trade, index) => (
            <div key={index} className={styles.historyItem}>
              <div className={styles.tradeHeader}>
                ...
                <span className={`${styles.status} ${styles[trade.status]}`}>{trade.status}</span>
              </div>
  ...
  const renderSettings = () => (
    <div className={styles.settingsTab}>
      ...
      <div className={styles.settingsForm}>
        <div className={styles.settingGroup}>
          ...
          <input
            ...
            onChange={(e) =>
              setSettings((prev) => ({
                ...prev,
                min_profit_threshold: Number.parseFloat(e.target.value),
              }))}
          />
        </div>
        ...
        <div className={styles.settingGroup}>
          ...
          <input
            ...
            onChange={(e) =>
              setSettings((prev) => ({
                ...prev,
                max_trade_amount: Number.parseFloat(e.target.value),
              }))}
          />
        </div>
        ...
        <button className={styles.saveButton} onClick={handleUpdateSettings} disabled={loading}>
