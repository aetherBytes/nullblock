...
const DeFiDashboard: React.FC<DeFiDashboardProps> = ({ onClose }) => {
  const [activeView, setActiveView] = useState<'overview' | 'yield' | 'liquidity' | 'automation'>(
    'overview',
  );
  ...
  const [selectedProtocol, setSelectedProtocol] = useState<'all' | 'aave' | 'uniswap' | 'compound'>(
    'all',
  );
      ...
      <div className={styles.metricsGrid}>
        ...
        <div className={styles.metricCard}>
          ...
          <div className={styles.metricLabel}>
            {(
              ((defiMetrics?.totalEarnings || 0) / (defiMetrics?.totalDeposited || 1)) *
              100
            ).toFixed(2)}
            % return
          </div>
        ...
        <div className={styles.metricCard}>
          <h4>Net P&L</h4>
          <div
            className={`${styles.metricValue} ${(defiMetrics?.totalEarnings || 0) + (defiMetrics?.impermanentLoss || 0) - (defiMetrics?.gasSpent || 0) > 0 ? styles.positive : styles.negative}`}
          >
            $
            {(
              (defiMetrics?.totalEarnings || 0) +
              (defiMetrics?.impermanentLoss || 0) -
              (defiMetrics?.gasSpent || 0)
            ).toFixed(2)}
          </div>
              ...
              <div key={strategy.id} className={styles.positionCard}>
                <div className={styles.positionHeader}>
                  ...
                  <div className={styles.positionInfo}>
                    ...
                    <span className={styles.positionProtocol}>
                  ...
                  <div className={styles.positionMetrics}>
                    ...
                    <span className={styles.allocation}>
                      ${strategy.allocatedAmount.toLocaleString()}
                    </span>
                  </div>
                </div>
                ...
                <div className={styles.positionProgress}>
                  <div className={styles.progressBar}>
                    <div
                      ...
                      style={{
                        width: `${Math.min((strategy.earnings / strategy.allocatedAmount) * 100 * 10, 100)}%`,
                      }}
                    ></div>
                  </div>
  ...
  const renderYieldFarming = () => (
    <div className={styles.yieldView}>
      ...
      <div className={styles.strategiesList}>
        {yieldStrategies
          .filter(
            (strategy) => selectedProtocol === 'all' || strategy.protocol === selectedProtocol,
          )
          .map((strategy) => (
            <div key={strategy.id} className={styles.strategyCard}>
              <div className={styles.strategyHeader}>
                <div className={styles.strategyInfo}>
                  ...
                  <div className={styles.strategyDetails}>
                    ...
                    <span className={styles.strategyProtocol}>
                      {strategy.protocol.toUpperCase()}
                    </span>
                  </div>
                </div>
              ...
              <div className={styles.strategyStats}>
                ...
                <div className={styles.stat}>
                  ...
                  <span
                    className={`${styles.statValue} ${strategy.autoCompound ? styles.positive : styles.negative}`}
                  >
    ...
    <div className={styles.liquidityView}>
      <div className={styles.liquidityHeader}>
        ...
        <div className={styles.liquidityStats}>
          <span>
            Total Value: $
            {liquidityPositions.reduce((acc, pos) => acc + pos.totalValue, 0).toLocaleString()}
          </span>
          <span>
            24h Fees: +${liquidityPositions.reduce((acc, pos) => acc + pos.fees24h, 0).toFixed(2)}
          </span>
        </div>
      </div>
          ...
          <div key={position.id} className={styles.liquidityCard}>
            <div className={styles.liquidityHeader}>
              ...
              <div className={styles.rangeStatus}>
                <span
                  className={`${styles.rangeIndicator} ${position.range.inRange ? styles.inRange : styles.outOfRange}`}
                >
            ...
            <div className={styles.liquidityMetrics}>
              ...
              <div className={styles.metric}>
                ...
                <span
                  className={`${styles.metricValue} ${position.impermanentLoss >= 0 ? styles.positive : styles.negative}`}
                >
          ...
          <div key={strategy.id} className={styles.automationCard}>
            ...
            <div className={styles.automationMetrics}>
              ...
              <div className={styles.metric}>
                ...
                <span
                  className={`${styles.metricValue} ${strategy.profitLoss >= 0 ? styles.positive : styles.negative}`}
                >
