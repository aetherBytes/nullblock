...
start-mac:
    ...
    @docker run -d --name nullblock-postgres-erebus -p 5440:5432 -e POSTGRES_DB=erebus -e POSTGRES_USER=postgres -e POSTGRES_PASSWORD=postgres_secure_pass postgres:15-alpine
    @docker run -d --name nullblock-postgres-agents -p 5441:5432 -e POSTGRES_DB=agents -e POSTGRES_USER=postgres -e POSTGRES_PASSWORD=postgres_secure_pass postgres:15-alpine
    @echo "📦 Starting Redis..."
...
start-linux:
    ...
    @docker rm -f nullblock-postgres-erebus nullblock-postgres-agents nullblock-redis nullblock-zookeeper nullblock-kafka 2>/dev/null || true
    @echo "📦 Starting PostgreSQL databases..."
    @docker run -d --name nullblock-postgres-erebus --network=host -e POSTGRES_DB=erebus -e POSTGRES_USER=postgres -e POSTGRES_PASSWORD=postgres_secure_pass -e PGPORT=5440 postgres:15-alpine -p 5440
    @docker run -d --name nullblock-postgres-agents --network=host -e POSTGRES_DB=agents -e POSTGRES_USER=postgres -e POSTGRES_PASSWORD=postgres_secure_pass -e PGPORT=5441 postgres:15-alpine -p 5441
    @echo "📦 Starting Redis..."
